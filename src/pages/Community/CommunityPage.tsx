...
const posts = [
  ...
  },
  {
    ...
    images: [{ src: postImageTwo, fit: "object-cover" }, ...carouselImages.slice(1)],
  ...
  },
  {
    ...
    images: [carouselImages[2], carouselImages[3], carouselImages[0], carouselImages[1]],
    products: tagImagesOne,
  },
  {
    author: "오늘의잔향",
    ...
    images: [carouselImages[3], carouselImages[1], { src: postImageTwo, fit: "object-cover" }, carouselImages[2]],
    products: [tagImagesTwo[0], tagImagesOne[0], tagImagesOne[2]],
  },
] as const;

const commentThreads: Record<string, { author: string; avatar: string; text: string; likes: number }[]> = {
  "review-1": [
    { author: "향기수집가", avatar: avatarHaesu, text: "베이지 룩이랑 향 조합이 정말 잘 어울려요!", likes: 18 },
    { author: "은은한하루", avatar: avatarYeeun, text: "오 로즈 레이어링 저도 따라 해볼게요.", likes: 9 },
  ],
  "review-2": [
    { author: "포근한잔향", avatar: avatarYeeun, text: "종이 향처럼 깨끗한 머스크라니 궁금해요.", likes: 24 },
    { author: "muskday", avatar: avatarHaesu, text: "출근할 때 뿌리기에도 부담 없을 것 같아요!", likes: 11 },
  ],
  "review-3": [
    { author: "비오는향", avatar: avatarHaesu, text: "습한 날 우디 향 조합 너무 좋아요.", likes: 15 },
    { author: "scent.zip", avatar: avatarYeeun, text: "잔향이 차분해서 가을에도 잘 어울리겠네요.", likes: 7 },
  ],
  "review-4": [
    { author: "바닐라구름", avatar: avatarYeeun, text: "니트 입는 날 꼭 시도해보고 싶어요.", likes: 21 },
    { author: "오늘의향", avatar: avatarHaesu, text: "달콤하지만 답답하지 않은 조합 같아요!", likes: 13 },
  ],
  "question-1": [
    { author: "과일향러버", avatar: avatarHaesu, text: "로에베 아구아 엘라 추천해요. 상큼하고 맑아요!", likes: 32 },
    { author: "향수초보", avatar: avatarYeeun, text: "올리브영에서 아쿠아 계열도 시향해보세요.", likes: 12 },
  ],
  "question-2": [
    { author: "office_scent", avatar: avatarYeeun, text: "클린 웜 코튼은 사무실에서도 부담 없었어요.", likes: 27 },
    { author: "잔향노트", avatar: avatarHaesu, text: "가볍게 한 번만 뿌리면 블랑쉬도 괜찮아요.", likes: 16 },
  ],
};

export function CommentSheet({ onClose, threadId }: { onClose: () => void; threadId: string }) {
  const [message, setMessage] = useState("");
  const [comments, setComments] = useState(() => (commentThreads[threadId] ?? []).map((comment, index) => ({
    ...comment,
    id: `${threadId}-${index}`,
    isLiked: false,
  })));
  ...
  return (
    <div className="fixed inset-0 z-[80]" onClick={onClose}>
      <section
        ...
        role="dialog"
      >
        <button aria-label="댓글창 닫기" className="mx-auto mt-3.5 h-4 w-16" onClick={onClose} type="button">
        ...
        <div className="flex-1 space-y-7 overflow-y-auto px-5 py-[30px]">
          {comments.map((comment) => (
            <article className="flex items-start gap-3.5" key={comment.id}>
              <img className="size-[26px] rounded-full object-cover" src={comment.avatar} alt="" />
              <div>
                <p className="text-sm tracking-[-0.02em] text-grey">{comment.author}</p>
                <p className="mt-1 text-sm tracking-[-0.02em]">{comment.text}</p>
                <div className="mt-[17px] flex items-center text-xs font-medium">
                  <button
                    ...
                    onClick={() => setComments((current) => current.map((item) => (
                      item.id === comment.id
                    ...
                    type="button"
                  >
                    <Heart fill={comment.isLiked ? "currentColor" : "none"} size={14} strokeWidth={1.5} />
                    {comment.likes}
                  </button>
                </div>
              </div>
        ...
        >
          <input
            ...
            value={message}
          />
          <button aria-label="댓글 전송" className="flex size-[50px] shrink-0 items-center justify-center rounded-full bg-off-black text-white" type="submit">
            <Send size={18} />
          </button>
        </form>
      </section>
  ...
  return <article className="px-5" data-node-id={`community-post-${index + 1}`}>
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-2.5">
        <img className="size-[42px] rounded-full object-cover" src={post.avatar} alt="" />
        <div><p className="text-base font-medium tracking-[-0.02em]">{post.author}</p><p className="mt-[3px] text-xs font-medium tracking-[-0.02em] text-grey">{post.time}</p></div>
      </div>
    ...
    <div className="mt-6 flex flex-col items-center">
      <div className="no-scrollbar relative h-[430px] w-full snap-x snap-mandatory overflow-x-auto overflow-y-hidden rounded-lg" onScroll={(event) => setActiveSlide(Math.round(event.currentTarget.scrollLeft / 400))}>
        <div className="flex h-full w-max gap-2.5">
          {post.images.map(({ src, fit }) => <img className={`h-full w-[390px] shrink-0 snap-start rounded-lg ${fit}`} src={src} alt="향수 커뮤니티 리뷰" key={src} />)}
        </div>
          ...
          >
            <div className="flex items-center gap-2 rounded-lg border-[0.8px] border-light-grey bg-white p-2">
              <span className="flex size-8 items-center justify-center overflow-hidden rounded-[6px] bg-[#ededed]">
                <img className="h-[39px] w-[36px] object-cover" src={productOverlayThumbnail} alt="" />
              </span>
              <span className="flex flex-col gap-1 whitespace-nowrap text-xs leading-none tracking-[-0.02em]">
                <span className="font-medium text-[#888]">Jo Malone London</span>
                <span className="font-bold text-[#171717]">블랙베리 앤 베이 코롱</span>
              </span>
            </div>
            <img aria-hidden="true" className="absolute left-1/2 top-full size-3 -translate-x-1/2 -translate-y-1/2" src={index === 1 ? plusImageTwo : plusImageOne} alt="" />
          </div>
        ))}
      </div>
      <div className="relative mt-3 h-0.5 w-[160px] bg-grey"><div className="absolute left-0 top-0 h-0.5 w-10 bg-off-black transition-transform duration-200" style={{ transform: `translateX(${activeSlide * 40}px)` }} /></div>
    </div>
    <div className="mt-6">{actions}</div>
    <div className="mt-6">
      <span className="inline-flex h-5 items-center rounded bg-[#ffede6] px-2 text-[10px] font-semibold tracking-[-0.04em] text-[#ff4800]">추천해요</span>
      <h3 className="mt-1.5 text-base font-semibold tracking-[-0.02em]">{post.title}</h3>
      <p className="mt-2.5 text-sm leading-normal tracking-[-0.02em] text-[#4d4d4d]">{post.body}</p>
      <div className="mt-3 flex gap-2.5 whitespace-nowrap text-sm tracking-[-0.02em] text-[#4d4d4d]">{post.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>
    </div>
    <div className="mt-6 flex h-[70px] items-center justify-between overflow-hidden">
      <div className="flex gap-3">{post.products.map(({ src }) => <div className="flex size-[68px] items-center justify-center rounded-lg border border-light-grey bg-white" key={src}><img className="h-[46px] w-[32px] object-contain" src={src} alt="향수" /></div>)}</div><ChevronRight size={18} className="text-grey" />
    </div>
    {isCommentsOpen && <CommentSheet onClose={() => setIsCommentsOpen(false)} threadId={`review-${index + 1}`} />}
  </article>;
}
